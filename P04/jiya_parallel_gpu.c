...
int main() {
    ...
    while (residual > MAX_RESIDUAL && iteration <= MAX_ITER) {
        ...
        #pragma omp target teams distribute parallel for collapse(2) map(tofrom: T[:(N+2)*(N+2)], T_new[:(N+2)*(N+2)])
        ...
        #pragma omp target teams distribute parallel for reduction(max : local_residual) collapse(2) map(tofrom: T[:(N+2)*(N+2)], T_new[:(N+2)*(N+2)])
        for (unsigned i = 1; i <= N; i++) {
            for (unsigned j = 1; j <= N; j++) {
